...
import { usePublicGalleryItems, useGalleryCategories } from "../../hooks/useGallery";
...
const Gallery = () => {
  ...
  const {
    ...
    error: itemsError
  } = usePublicGalleryItems(selectedCategory);
  ...
  // Memoized categories with "all" option
  const categories = useMemo(() => {
    return ["all", ...categoriesData];
  }, [categoriesData]);

  // Filter items for the current category (already handled in the query)
          ...
          <div className="text-center">
            <div className="bg-red-50 border border-red-200 rounded-lg p-8 max-w-md mx-auto">
              ...
              <p className="text-red-600 text-sm">
                {itemsError?.response?.data?.message || "Please try again later."}
              </p>
              <button
